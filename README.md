...
│ MCP Host │ ───────▶ │  MCP Server      │ ───────▶ │  Shared Daemon   │ ───────────▶ │ 飞书  │
│ (LLM)    │ ◀─────── │  (ask/send)      │ ◀─────── │  (长连接+事件)    │ ◀─────────── │ 用户  │
└──────────┘          └──────────────────┘   IPC    └──────────────────┘             └──────┘
```
...
```
MCP Host 进程                          共享 Daemon 进程
┌─────────────────────┐               ┌─────────────────────────────┐
│  MCP Server (stdio) │    HTTP IPC   │  Shared Long-Connection     │
│  ┌───────────────┐  │ ────────────▶ │  ┌───────────────────────┐  │
│  │ ask/send 工具  │  │ ◀──────────── │  │ 飞书 WebSocket 长连接  │  │
│  └───────────────┘  │               │  └───────────────────────┘  │
│                     │               │  ┌───────────────────────┐  │
│  daemon bootstrap:  │               │  │ 事件拦截 & 分发        │  │
│  自动发现/启动daemon │               │  └───────────────────────┘  │
└─────────────────────┘               │  ┌───────────────────────┐  │
                                      │  │ Per-chat Ask 队列      │  │
多个 MCP Host 进程                     │  └───────────────────────┘  │
可共享同一个 Daemon ──────────────────▶ │  空闲自动退出，按需自动拉起  │
                                      └─────────────────────────────┘
```
