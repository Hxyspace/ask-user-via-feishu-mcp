...
2. **local通道** -> 本地交流，通过copilot终端和`ask_user`工具交流回复。
...
2. 当我让你切到本地通道交互时，你必须通过`ask_user`工具跟我交流，每次任务完成或者需要决策时都通过`ask_user`询问我，除非我主动停止或让你切到别的通道。永远都不能主动结束对话，除非我让你结束。`ask_user`通知消息并给出决策选项然后等待。可以先输出长的总结消息，紧接着通过`ask_user`等待询问下一步指示。
